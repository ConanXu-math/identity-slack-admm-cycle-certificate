...
变量为
\[
x\in\mathbb R^{n_x},\qquad
y\in\mathbb R^{n_y},\qquad
...
给定函数
\[
f:\mathbb R^{n_x}\to\mathbb R,\qquad
g:\mathbb R^{n_y}\to\mathbb R,
\]
满足
\[
f\in C^1(\mathbb R^{n_x}),\qquad
g\in C^1(\mathbb R^{n_y}),
\]
并且分别强凸：存在常数
\[
\mu_f>0,\qquad \mu_g>0,
\]
使得对任意 \(u,v\in\mathbb R^{n_x}\)，
\[
f(u)\ge
f(v)+\langle\nabla f(v),u-v\rangle
+\frac{\mu_f}{2}\|u-v\|^2,
\]
对任意 \(u,v\in\mathbb R^{n_y}\)，
\[
g(u)\ge
g(v)+\langle\nabla g(v),u-v\rangle
+\frac{\mu_g}{2}\|u-v\|^2.
...
线性约束数据满足
\[
A\in\mathbb Q^{m\times n_x},\qquad
B\in\mathbb Q^{m\times n_y},\qquad
b\in\mathbb Q^m,
\]
以及
\[
...
x^{k+1}
=
\operatorname*{argmin}_{x\in\mathbb R^{n_x}}
\left\{
f(x)
-\langle\lambda^k,Ax+By^k+z^k-b\rangle
+\frac12\|Ax+By^k+z^k-b\|^2
\right\},
\]
\[
y^{k+1}
=
\operatorname*{argmin}_{y\in\mathbb R^{n_y}}
\left\{
g(y)
-\langle\lambda^k,Ax^{k+1}+By+z^k-b\rangle
+\frac12\|Ax^{k+1}+By+z^k-b\|^2
\right\},
\]
\[
z^{k+1}
...
\]
\[
\nabla g(y^\star)-B^\top\lambda^\star=0,
\]
\[
...
(x,y)\mapsto Ax+By
\]
满射到 \(\mathbb R^m\)。任取严格正向量 \(\bar z>0\)，均可求得 \((\bar x,\bar y)\) 满足
\[
A\bar x+B\bar y=b-\bar z.
\]
因此原不等式形式
\[
Ax+By\le b
...
这里要求完整状态序列收敛到 KKT 点。目标函数值、原始可行性残差、相邻迭代差值以及某个子序列的收敛，可以作为证明中的中间结论。
...
1. \(f\in C^1(\mathbb R^{n_x})\) 且强凸；
2. \(g\in C^1(\mathbb R^{n_y})\) 且强凸；
3. \(A,B,b\) 的全部分量均为有理数；
